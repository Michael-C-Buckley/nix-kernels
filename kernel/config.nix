...
in {
  ...
  # v4 = 2017+ (AVX512)
  X86_64_VERSION = freeform x86version; # My default is v3

  ### Profile-based RCU Configuration
  RCU_EXPERT = yes;
  RCU_FANOUT = freeform (
    if cfg.cpuGovernor == "performance" && cfg.preemptModel == "full"
    then "32"  # Lower latency for performance
    else "64"  # Higher throughput for server/balanced
  );
  RCU_FANOUT_LEAF = freeform "16";
  RCU_BOOST = yes;
  RCU_BOOST_DELAY = freeform (
    if cfg.cpuGovernor == "performance" && cfg.preemptModel == "full"
    then "0"    # Immediate boosting for low latency
    else if cfg.preemptModel == "dynamic"
    then "100"  # Moderate delay for balanced
    else "500"  # Longer delay for server throughput
  );
  RCU_EXP_KTHREAD = (
    if cfg.preemptModel == "voluntary"
    then no   # Disable for server - can hurt throughput
    else yes  # Enable for balanced/performance
  );
  ...
  ### Profile-based Debug Configuration
  # Performance profile enables debug info for observability tools (bpftrace, perf)
  # Server/Balanced disable for smaller kernel size and faster builds
  DEBUG_INFO = mkForce (
    if cfg.cpuGovernor == "performance" && cfg.preemptModel == "full"
    then yes
    else no
  );
  DEBUG_INFO_DWARF4 = mkForce (
    if cfg.cpuGovernor == "performance" && cfg.preemptModel == "full"
    then yes
    else no
  );
  DEBUG_INFO_BTF = mkForce (
    if cfg.cpuGovernor == "performance" && cfg.preemptModel == "full"
    then yes
    else no
  );
  DEBUG_INFO_REDUCED = mkForce no; # Always disabled for smaller size
